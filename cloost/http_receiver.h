 ...
 *
 *  Last-modified: Mon 25 Oct 2010 02:46:00 JST
 */
/* ------------------------------------------------------------------------- */
#ifndef CLOOST_HTTP_RECEIVER_H
#define CLOOST_HTTP_RECEIVER_H
...
namespace cloost {
	namespace net {
		...
		public:
			...
			response operator()(SocketWrapper& s, const request& req) {
				response dest;
				boost::asio::streambuf buf;
				this->read_header(s, buf, req, dest);
				this->read_body(s, buf, req, dest);
				return dest;
			}
		...
		private:
			...
			void read_header(SocketWrapper& s, MutableBuffer& buf, const request& /* req */, response& dest) {
				...
				while (std::getline(input, header) && !header.empty() && header != "\r") {
					if (header.at(0) == 0x09 || header.at(0) == 0x20) {
						response::header_iterator pos = dest.headers().find(key);
			...
			/* ------------------------------------------------------------- */
			template <class SocketWrapper, class MutableBuffer>
			void read_body(SocketWrapper& s, MutableBuffer& buf, const request& /* req */, response& dest) {
				response::const_header_iterator pos;
